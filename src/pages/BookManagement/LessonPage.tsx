...
import { ChevronLeft, ChevronRight, Clock, CheckCircle, Save, Boxes, BookOpen } from 'lucide-react'
import { BOOK_MANAGEMENT_LESSONS } from '../../data/book-management-curriculum'
import { useProgress } from '../../hooks/useProgress'
import { useAuth } from '../../hooks/useAuth'
import MarkdownContent from '../../components/MarkdownContent'

export default function BookManagementLessonPage() {
  const { id } = useParams<{ id: string }>()
  const lesson = BOOK_MANAGEMENT_LESSONS.find(l => l.id === id)
  const idx = BOOK_MANAGEMENT_LESSONS.findIndex(l => l.id === id)
  const prev = BOOK_MANAGEMENT_LESSONS[idx - 1]
  const next = BOOK_MANAGEMENT_LESSONS[idx + 1]

  const { user } = useAuth()
  const { progress, saveProgress } = useProgress(user?.id, 'book-management')
    ...
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <div className="fixed top-14 left-0 right-0 z-40 h-0.5 bg-[var(--border)]">
        <div
          ...
          style={{ width: `${((idx + 1) / BOOK_MANAGEMENT_LESSONS.length) * 100}%` }}
        />
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center gap-2 text-xs text-[var(--muted)] mb-8">
          <Link to="/book-management" className="hover:text-white transition-colors flex items-center gap-1">
            <Boxes size={12} /> Book Management
          </Link>
          <span>/</span>
        ...
        <div className="mb-8">
          <div className="flex flex-wrap items-center gap-3 mb-5">
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-teal-500/15 border border-teal-500/30 text-teal-400">
              Bài {String(lesson.unit).padStart(2, '0')} / {BOOK_MANAGEMENT_LESSONS.length}
            </span>
            ...
            <span className="flex items-center gap-1.5 text-xs text-[var(--muted)] ml-auto">
              <BookOpen size={12} /> {totalDone}/{BOOK_MANAGEMENT_LESSONS.length} bài xong
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {lesson.tags.map(t => (
              <span key={t} className="text-xs px-2.5 py-1 rounded-full border border-[var(--border)] text-[var(--muted)] hover:border-teal-600/40 transition-colors">
                #{t}
              </span>
            ))}
          </div>
        ...
        {tab === 'read' && (
          <>
            <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-6 md:p-8 mb-8">
              <MarkdownContent content={lesson.content} accentColor="teal" />
            </div>

            {!isRead ? (
              <div className="sticky bottom-6 flex justify-center">
                <button onClick={markRead}
                  className="flex items-center gap-2.5 px-8 py-3.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 rounded-2xl font-semibold text-sm shadow-2xl shadow-green-500/30 transition-all hover:scale-105">
        ...
        {tab === 'notes' && (
          <div>
            ...
            <button onClick={saveNotes}
              className="flex items-center gap-2 px-6 py-2.5 bg-teal-600 hover:bg-teal-500 rounded-xl text-sm font-medium transition-all hover:shadow-lg hover:shadow-teal-500/20">
        ...
        <div className="flex items-stretch justify-between gap-4 mt-12 pt-8 border-t border-[var(--border)]">
          {prev ? (
            <Link to={`/book-management/${prev.id}`}
          ...
          {next ? (
            <Link to={`/book-management/${next.id}`}
